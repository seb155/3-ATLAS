...
# ============================================================================
# Project Detection (from session state - logical tracking)
# Reads current_project from session-state.json instead of using pwd
# This allows tracking the actual project being worked on, not just
# the directory where Claude Code was launched.
# ============================================================================
STATE_FILE="$HOME/.claude/session-state.json"

# Default values (fallback to ATLAS)
PROJECT_EMOJI="🏛️"
PROJECT_NAME="ATLAS"

# Read project from session state file if available
if [ -f "$STATE_FILE" ] && command -v jq &> /dev/null; then
    TRACKED_PROJECT=$(jq -r '.current_project // empty' "$STATE_FILE" 2>/dev/null)
    TRACKED_EMOJI=$(jq -r '.project_emoji // empty' "$STATE_FILE" 2>/dev/null)

    # Use tracked values if they exist and are not null/empty
    if [ -n "$TRACKED_PROJECT" ] && [ "$TRACKED_PROJECT" != "null" ]; then
        PROJECT_NAME="$TRACKED_PROJECT"
    fi
    if [ -n "$TRACKED_EMOJI" ] && [ "$TRACKED_EMOJI" != "null" ]; then
        PROJECT_EMOJI="$TRACKED_EMOJI"
    fi
fi
