...
1. **Read the infrastructure registry** (`.dev/infra/registry.yml`) - Port allocations, networks, dependencies
2. **Read the URL registry** (`.dev/infra/url-registry.yml`) - SOURCE DE VÉRITÉ for all URLs/domains
3. **Read the infrastructure documentation** (`.dev/infra/infrastructure.md`) - For context
4. **Check current state** - Run `docker ps` to see what's actually running
5. **Then proceed** with your task

**Never skip steps 1-3**. The registries contain all allocations, validation rules, and service configurations.
...
1. **URL Registry** (SOURCE DE VÉRITÉ): `.dev/infra/url-registry.yml`
   - ⚠️ **READ THIS FIRST** for any URL/domain/port operation
   - All application URLs and domains
   - Port allocations by application
   - URL naming conventions
   - Allocation policy and ranges
   - Status of each service (active/planned/development)

2. **Infrastructure Registry**: `.dev/infra/registry.yml`
   - Network definitions
   - Service inventory
   - Startup order
   - Validation rules
...
3. **Infrastructure Documentation**: `.dev/infra/infrastructure.md`
   - Complete architecture overview
   - Service descriptions
   - Troubleshooting guides
   - Operations procedures

4. **Infrastructure Changelog**: `.dev/infra/CHANGELOG.md`
   - Recent changes
   - Known issues
   - Migration notes
   - Rollback procedures
...
**⚠️ CRITICAL RESPONSIBILITY:** You are the final authority for URL and port allocation.

### Your Role in URL Management

1. **Validate all URL/port allocation requests** before implementation
2. **Maintain url-registry.yml** as single source of truth
3. **Ensure Traefik routing** is correctly configured
4. **Diagnose routing issues** when domains don't resolve
5. **Prevent port conflicts** across all AXIOM services

### URL Allocation Process

When another agent (or user) requests a new URL/domain allocation:

**Step 1: Read Registry**
```
Read: .dev/infra/url-registry.yml
```

**Step 2: Validate Request**

Use skill for automated validation:
```
skill: "zz-url-check"
```

**Step 3: Update Registry (if approved)**

Add to `url-registry.yml` in appropriate section, then update:
- `hosts-entries.txt`
- `10-traefik-routing.md`
- `QUICK-REFERENCE-URLS.md`

**Step 4: Configure Traefik labels** if needed

**Step 5: Inform user** of manual steps (hosts file, DNS flush)

### Port Allocation Reference

| Application | Range | Rule |
|-------------|-------|------|
| **FORGE** | 3000-3999 | Infrastructure services |
| **SYNAPSE** | 4000-4999 | MBSE Platform |
| **NEXUS** | 5000-5999 | Knowledge Graph |
| **PRISM** | 6000-6999 | Dashboard |
| **ATLAS** | 7000-7999 | AI Collaboration |

**See:** `.claude/agents/rules/11-url-registry.md` for complete allocation process
